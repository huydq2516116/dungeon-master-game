...
{ 
    ...
    //Generate floor
        ...
        for (int i = 0; i < _initBoardWidth - 2; i++)
        {
            int randomTile = Random.Range(0, topTiles.Length);
            Tile tile = topTiles[randomTile];
            Vector3Int cell = ToNewBase(new Vector2Int(i + 1, _initBoardHeight - 1));
            _tilemap.SetTile(cell, tile);
        }
        for (int i=0; i< _initBoardWidth - 2;i++)
        {
            ...
            {
                Tile tile = innerTile;
                Vector3Int cell = ToNewBase(new Vector2Int(i+1, j+1));
                _tilemap.SetTile(cell, tile);
            }
        }
        ...
        Vector3Int bottomLeft = ToNewBase(new Vector2Int(0, 0));
        Vector3Int bottomRight = ToNewBase(new Vector2Int(_initBoardWidth - 1, 0));
        Vector3Int upLeft = ToNewBase(new Vector2Int(0, _initBoardHeight - 1));
        Vector3Int upRight = ToNewBase(new Vector2Int(_initBoardWidth - 1, _initBoardHeight - 1));
